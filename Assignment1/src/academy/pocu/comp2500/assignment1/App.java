...
public class App {
    public App(Registry registry) {
        ...
        registry.registerAuthorFilterSetter("Blog", "setAuthorFilterOrNull"); //: 작성자 필터를 설정하는 메서드를 등록한다.
        registry.registerTagFilterSetter("Blog", "setTagsFilter"); //: 태그 필터를 설정하는 메서드를 등록한다.
        registry.registerPostOrderSetter("Blog", "setSortingType"); //: 블로그 글의 정렬 방법을 설정하는 메서드를 등록한다.
        registry.registerPostListGetter("Blog", "getFilteredPostsAndSort"); // 블로그 글 목록을 가져오는 메서드를 등록한다.

        registry.registerBlogCreator("Blog"); // : 블로그를 생성하는 생성자를 등록한다.
        registry.registerPostAdder("Blog", "addPost"); // 블로그에 글을 추가하는 메서드를 등록한다.

        registry.registerPostTitleUpdater("Post", "changeTitleCheckIsAuthor"); //: 발행된 블로그 글의 제목을 바꾸는 메서드를 등록한다.
        registry.registerPostBodyUpdater("Post", "changeBodyCheckIsAuthor"); //: 발행된 블로그 글의 본문을 바꾸는 메서드를 등록한다.
        ...
        registry.registerCommentUpdater("PostComment", "changeBodyCheckIsAuthor"); //: 댓글의 내용을 바꾸는 메서드를 등록한다.
        registry.registerSubcommentUpdater("PostComment", "changeBodyCheckIsAuthor"); //: 하위 댓글의 내용을 바꾸는 메서드를 등록한다.
